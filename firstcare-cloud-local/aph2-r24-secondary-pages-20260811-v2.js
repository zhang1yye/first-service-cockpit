(() => {
  ...
  const RELEASE = 'r24-secondary-pages-20260811-v2'
  const ROUTE_CLASSES = [
    'aph-r24-command',
    'aph-r24-projects',
    'aph-r24-ai-alerts',
  ...
  function enhanceReport(main) {
    ...
    ;[...layout.children].forEach(child => {
      ...
      if (text.includes('领导版') && text.includes('经营版')) {
        child.classList.add('aph-r24-report-mode-actions')
        modeActions = child
      }
      if (text.includes('片区筛选') && text.includes('打印/导出PDF')) {
        child.classList.add('aph-r24-report-filter-actions')
        filterActions = child
      }
      if (text.includes('项目经营事实未发布，月报未生成')) child.classList.add('aph-r24-report-empty')
    })

    // 导出动作和版本动作统一放在右侧，筛选器单独留在左侧。
    const exportActions = filterActions?.querySelector('.no-print')
    if (modeActions && exportActions && exportActions.parentElement !== modeActions) modeActions.append(exportActions)
  }
