(() => {
  ...
  const RELEASE = 'r6-operations-20260810-v6'
  ...
  function relocateSourceFreshness() {
    ...
    const currentPanel = document.querySelector('body > .aph-source-freshness')
    if (pathname === '/') {
      cleanupSourceFreshnessLayout()
      currentMain?.style.removeProperty('padding-top')
      if (currentPanel) currentPanel.style.setProperty('display', 'none', 'important')
      return
    }
    if (pathname !== '/') {
      cleanupSourceFreshnessLayout()
      if (currentPanel && ['/command', '/collection'].includes(pathname)) currentPanel.style.setProperty('display', 'none', 'important')
      if (currentMain && ['/command', '/collection'].includes(pathname)) {
        currentMain.style.setProperty('padding-top', window.matchMedia('(max-width: 640px)').matches ? '64px' : '108px', 'important')
      }
      return
    }
  }
