(() => {
  ...
  function hideCollectionAgingColumns(pathname) {
    ...
    main.querySelectorAll('table').forEach(table => {
      ...
      // 服务中心明细保留全部行，由统一滚动容器控制高度和粘性表头。
      const scrollContainer = table.closest('.table-scroll')
      if (scrollContainer) {
        scrollContainer.style.removeProperty('max-height')
        scrollContainer.classList.add('aph-service-center-scroll')
        scrollContainer.dataset.aphCollectionRowsScrollable = 'true'
      }
    })
  }
  ...
  function enhanceServiceCenterDetailScrolling(main, pathname) {
    ...
    main.querySelectorAll('table').forEach(table => {
      ...
      if (!host) return
      host.classList.add('aph-service-center-scroll')
      host.setAttribute('tabindex', '0')
      host.setAttribute('aria-label', '服务中心明细，可纵向和横向滚动')
    })
  }
