...
function setArrearsView(value){
  const allowed=new Set(['overview','details','workbench']);const view=allowed.has(value)?value:'overview'
  sessionStorage.setItem('arrears-active-view',view)
  document.querySelectorAll('[data-arrears-view]').forEach(node=>{node.hidden=node.dataset.arrearsView!==view})
  document.querySelectorAll('[data-arrears-tab]').forEach(button=>{const active=button.dataset.arrearsTab===view;button.setAttribute('aria-selected',String(active));button.tabIndex=active?0:-1})
}
...
async function api(path, options={}){
  ...
  if(response.status===401){localStorage.removeItem('cockpit_token');redirectToLogin();throw new Error('登录已失效')}
  const type=response.headers.get('content-type')||''
  const body=type.includes('application/json')?await response.json():await response.blob()
  if(!response.ok) throw new Error(body?.error||`请求失败 HTTP ${response.status}`)
  return {body,response}
}
function statusBadge(value){const map={parsed:['已校验','warn'],blocked:['已阻断','bad'],analyzing:['AI分析中','warn'],analyzed:['AI完成','ok'],rule_only:['规则结果','warn'],revoked:['已撤回','bad'],archive_deleted:['原始密文已删除','bad'],ok:['正常','ok'],partial:['部分通过','warn'],rejected:['证据门禁拒绝','bad'],failed:['失败','bad']};const cfg=map[value]||[value||'待分析',''];return el('span',cfg[0],`status ${cfg[1]}`)}
...
function renderOverview(){
  ...
  const empty=$('overviewEmpty');const panels=$('overviewPanels');const detailEmpty=$('detailEmpty')
  if(!ready){
    panels.classList.add('hidden');detailEmpty.classList.remove('hidden');empty.classList.remove('hidden');empty.replaceChildren(el('span','00','empty-index'))
    const copy=el('div');copy.append(el('strong','暂无可用于经营分析的有效批次'));copy.append(el('p',overview?.revokedBatchCount?`当前仅有${overview.revokedBatchCount}个已撤回批次，已按真实性门禁排除。请切换至批次工作台，创建具备有效原始证据的新正式批次。`:'请切换至批次工作台，上传欠费台账和企小码沟通记录，完成校验后再生成经营总览。'));empty.append(copy);setArrearsView(sessionStorage.getItem('arrears-active-view'));return
  }
  empty.classList.add('hidden');detailEmpty.classList.add('hidden');panels.classList.remove('hidden')
  ...
  setArrearsView(sessionStorage.getItem('arrears-active-view'))
}
...
function renderBatches(){
  ...
  state.batches.forEach(batch=>{
    ...
    else if(!batch.archive_deleted_at){wrap.append(button('恢复批次','secondary',()=>restoreBatch(batch.id)));wrap.append(button('删除原始密文','danger',()=>deleteArchive(batch.id)))}
    else wrap.append(statusBadge('archive_deleted'))
    actions.append(wrap);tr.append(id,project,size,validation,ai,actions);tbody.append(tr)
  })
}
async function viewBatch(id){
  try{const {body}=await api(`/api/arrears/batches/${id}`);state.selectedBatch=body;setArrearsView('workbench');$('resultPanel').classList.remove('hidden');$('resultTitle').textContent=`批次 #${id} · ${body.batch.project_name}`;renderResults(body);$('resultPanel').scrollIntoView({behavior:'smooth',block:'start'})}catch(error){showToast(error.message,true)}
}
...
document.querySelectorAll('[data-arrears-tab]').forEach(node=>node.addEventListener('click',()=>setArrearsView(node.dataset.arrearsTab)))
...
if(userLabel) userLabel.textContent=user.displayName||user.username||'当前账号'
setArrearsView(sessionStorage.getItem('arrears-active-view'))
