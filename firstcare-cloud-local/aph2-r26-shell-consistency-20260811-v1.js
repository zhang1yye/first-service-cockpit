/* 统一驾驶舱 SPA、独立管理页和历史入口，旧审核地址回到内置 APH 审核页。 */
(() => {
  'use strict'

  const RELEASE = 'r26-shell-consistency-20260811-v1'
  ...
  function refreshSystemLinks(root = document) {
    root.querySelectorAll('a[href="/admin"],a[href="/admin/"]').forEach((link) => {
      if (!link.closest('.aph-exact-sidebar, .aph-page-tabs, header')) return
  ...
  function bindHardNavigation() {
    document.addEventListener('click', (event) => {
      ...
      const nextPath = FULL_PAGE_ROUTES.get(target.pathname) || LEGACY_ROUTES.get(target.pathname)
      if (!nextPath) return
  ...
  document.documentElement.dataset.aphShellConsistency = RELEASE
  bindHardNavigation()
