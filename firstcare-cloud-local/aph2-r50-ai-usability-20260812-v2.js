...
(() => {
  ...
  const RELEASE = 'r50-ai-usability-20260812-v2'
  ...
  function ensureAssistantRecovery() {
    ...
    messages.querySelectorAll('.north-ai-message-assistant').forEach(message => {
      ...
      const isUnavailable = Boolean(message.querySelector('.north-ai-meta .unavailable'))
      const isProjectGate = isUnavailable
        && /PROJECT_DATA_QUALITY_BLOCKED|项目经营数据尚未通过真实性门禁|真实项目经营数据尚未接入/.test(copy)
