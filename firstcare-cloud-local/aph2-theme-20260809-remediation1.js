(() => {
  ...
  const RELEASE = '20260809-remediation1'
  ...
  function setMetricUnknown(main, label) {
    exactTextElements(main, label).forEach(labelElement => {
      const card = metricCard(labelElement, main)
      if (!card) return
      const candidates = Array.from(card.querySelectorAll('strong,b,[class*="text-2xl"],[class*="text-3xl"],[class*="text-4xl"],span,div'))
  ...
  function navigateHost(href) {
    ...
    if (target.pathname === '/review') {
      openReviewSystem()
      return
    }
    if (target.pathname.startsWith('/arrears/')) {
  ...
  function ensureBusinessHeading(main, pathname) {
    ...
    const nativeHeadingRoutes = new Set(['/', '/projects'])
    const injected = main.querySelector('h1[data-aph-business-heading="true"]')
    if (nativeHeadingRoutes.has(pathname)) {
      injected?.remove()
    ...
    if (nativeHeadings.length) {
      injected?.remove()
      main.removeAttribute('data-aph-business-heading-pending')
      return
    }
    if (injected || main.dataset.aphBusinessHeadingPending === 'true') return
    main.dataset.aphBusinessHeadingPending = 'true'
    window.setTimeout(() => {
      if (!main.isConnected) return
      main.removeAttribute('data-aph-business-heading-pending')
      const existingNative = main.querySelectorAll('h1:not([data-aph-business-heading="true"])')
      const existingInjected = main.querySelector('h1[data-aph-business-heading="true"]')
      ...
      heading.dataset.aphBusinessHeading = 'true'
