/* R65：先确认当前账号的数据范围，再加载同源不可变主 SPA。 */
const RELEASE = 'r65-app-bootstrap-20260813-v1'
...
if (logicalRoute && !token) {
  ...
  if (logicalRoute) window.history.replaceState(window.history.state, '', '/collection')

  await Promise.resolve(window.__aphR65ScopeReady).catch(() => null)
  await import('/assets/cockpit-r65-simple-admin-center-scope-20260813-v1/app-G7HUEEER.js')
