(() => {
  ...
  function applyCollectionFooterSummary(root, summary) {
    ...
    const outstanding = Number(summary.collectionOutstanding)
    const rate = Number(summary.collectionRate)
    setTextPreservingIcons(cells[1], formatWan(receivable))
    setTextPreservingIcons(cells[2], formatWan(received))
    setTextPreservingIcons(cells[3], formatWan(outstanding))
    setTextPreservingIcons(cells[4].querySelector('.num') || cells[4], (
      Number.isFinite(rate) ? `${(rate * 100).toFixed(2)}%` : '—'
    ))
    if (footerRow.dataset.aphOfficialSummaryApplied !== 'true') {
      footerRow.dataset.aphOfficialSummaryApplied = 'true'
      footerRow.title = '华北合计采用与首页相同的绿仔官方汇总口径；收缴率直接使用官方项目率按本期应收加权，不以实收÷应收反推'
    }
  }
  ...
  function applyLvzaiSummaryToDom(root, pathname) {
    ...
    if (pathname === '/collection') {
      ...
      replaceLiveMetricValue(root, '未收金额', formatWan(outstanding))
