(() => {
  ...
  function hideCollectionAgingColumns(pathname) {
    ...
    main.querySelectorAll('table').forEach(table => {
      ...
      // 收缴率标题、筛选和视图菜单必须留在滚动区外；实际滚动统一委托给 table-only 容器。
      const detailShell = table.closest('.table-scroll')
      if (detailShell && detailShell.dataset.aphScrollScope !== 'table-only') {
        detailShell.style.removeProperty('max-height')
        detailShell.classList.remove('aph-service-center-scroll')
        detailShell.classList.add('aph-service-center-detail-shell')
        detailShell.dataset.aphCollectionRowsScrollable = 'delegated-to-table-only'
      }
    })
  }
  ...
  function setTextPreservingIcons(element, value) {
    if (!element) return
  ...
  function applyCollectionFooterSummary(root, summary) {
    const table = Array.from(root.querySelectorAll('table')).find(candidate => {
      const labels = Array.from(candidate.querySelectorAll('thead th'))
        .map(header => (header.textContent || '').replace(/\s+/g, ''))
      return labels.includes('服务中心') && labels.includes('收缴率')
    })
    ...
    setTextPreservingIcons(cells[1], formatWan(summary.collectionReceivable))
    setTextPreservingIcons(cells[2], formatWan(summary.collectionReceived))
    setTextPreservingIcons(cells[3], formatWan(summary.collectionOutstanding))
    const rate = Number(summary.collectionRate)
    setTextPreservingIcons(
      cells[4].querySelector('.num') || cells[4],
      Number.isFinite(rate) ? `${(rate * 100).toFixed(2)}%` : '—',
    )
    footerRow.dataset.aphHeatingCorrectedSummary = 'true'
    footerRow.title = '全部采用供暖期间修正后的华北总口径；片区筛选采用修正后实收÷应收'
  }

  function applyLvzaiSummaryToDom(root, pathname) {
    if (!root || !['/', '/collection'].includes(pathname)) return
    if (pathname === '/collection') {
      root.dataset.aphCollectionRateBasis = 'all-heating-corrected-area-amount'
      exactTextElements(root, '华北综合收缴率').forEach(label => {
        if (label.textContent !== '综合收缴率') label.textContent = '综合收缴率'
      })
      const areaFilter = root.querySelector('select')
      if (areaFilter && areaFilter.value !== '全部') return
      const summary = window.__aphLvzaiSummary
      if (!summary) return
      replaceLiveMetricValue(root, '应收总额', formatWan(summary.collectionReceivable))
      replaceLiveMetricValue(root, '实收总额', formatWan(summary.collectionReceived))
      replaceLiveMetricValue(root, '未收金额', formatWan(summary.collectionOutstanding))
      replaceLiveMetricValue(
        root,
        '综合收缴率',
        Number.isFinite(Number(summary.collectionRate))
          ? `${(Number(summary.collectionRate) * 100).toFixed(2)}%`
          : '—',
      )
      applyCollectionFooterSummary(root, summary)
      const label = exactTextElements(root, '综合收缴率')[0]
      if (label?.parentElement) {
        label.parentElement.title = '全部采用供暖修正后总口径63.14%；选择片区后按该片区修正后实收÷应收'
      }
      return
    }
    const summary = window.__aphLvzaiSummary
    ...
    try {
      ...
      page.innerHTML = `
        ...
        <section class="aph-project-table-card">
          ...
          <div class="aph-project-table-wrap aph-service-center-scroll" tabindex="0" aria-label="项目档案表格，可纵向和横向滚动">
  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (!response.ok && pathname === '/api/ai/monthly-report' && response.status === 409) {
      response.clone().json().then(payload => {
        window.__aphMonthlyReportBlocked = payload || { status: 409 }
        schedule()
      }).catch(() => {
        window.__aphMonthlyReportBlocked = { status: 409 }
        schedule()
      })
    }
  ...
  function navigateWithinApp(targetHref, sourceLink) {
    ...
    if (targetUrl.pathname === '/arrears') {
      window.location.assign(targetUrl.pathname)
      return
    }
    if (document.body.dataset.aphUnifiedView) {
  ...
  function normalizeOpenTabHref(href) {
    const legacyTabs = new Map([
      ['/collection?view=arrears', '/arrears'],
      ['/arrears/', '/arrears'],
      ['/system/', '/admin'],
      ['/system', '/admin'],
      ['/admin/', '/admin'],
    ...
    return legacyTabs.get(href) || href
  }

  function tabHrefForLocation(pathname, search = '') {
    if (pathname === '/admin' || pathname === '/system') return '/admin'
    if (pathname === '/review' && search.includes('view=workbench')) {
      return '/review?view=workbench'
    }
    return pathname
  }

  function dedupeOpenTabs(items) {
    const seen = new Set()
    return items.reduce((result, item) => {
      const normalized = { ...item, href: normalizeOpenTabHref(item.href) }
      if (seen.has(normalized.href)) return result
  ...
  function readOpenTabs() {
    try {
      ...
      return dedupeOpenTabs(stored.filter(item => (
        item
  ...
  function writeOpenTabs(items) {
    const normalizedItems = dedupeOpenTabs(items)
    const home = normalizedItems.find(item => item.href === '/') || { href: '/', label: '首页' }
    const recent = normalizedItems.filter(item => item.href !== '/').slice(-9)
  ...
  function enhanceCollectionDisclosure(main, pathname) {
    if (pathname !== '/collection' || !main) return
    const trendHeading = exactTextElements(main, '历史收缴趋势')[0]
    const detailHeading = exactTextElements(main, '华北各片区收缴明细')[0]
    const trend = trendHeading?.closest('.card-soft')
    const details = detailHeading?.closest('.card-soft')
    if (!trend || !details) return

    if (!main.querySelector('.aph-page-view-switch')) {
      const viewSwitch = document.createElement('div')
      viewSwitch.className = 'aph-page-view-switch'
      viewSwitch.dataset.aphMenuOutsideScroll = '1'
      viewSwitch.setAttribute('role', 'tablist')
      viewSwitch.setAttribute('aria-label', '收缴率页面视图')
      viewSwitch.innerHTML = '<button type="button" role="tab" data-collection-view="overview">经营概览</button><button type="button" role="tab" data-collection-view="details">项目明细</button>'
      trend.parentElement?.insertBefore(viewSwitch, trend)
      viewSwitch.addEventListener('click', event => {
        const button = event.target.closest('[data-collection-view]')
        if (!button) return
        window.sessionStorage.setItem('aph-collection-view', button.dataset.collectionView)
        enhanceCollectionDisclosure(main, pathname)
        if (button.dataset.collectionView === 'details') {
          window.requestAnimationFrame(() => viewSwitch.scrollIntoView({ block: 'start', behavior: 'smooth' }))
        }
      })
    }

    const view = window.sessionStorage.getItem('aph-collection-view') === 'details' ? 'details' : 'overview'
    main.querySelectorAll('[data-collection-view]').forEach(button => {
      const selected = button.dataset.collectionView === view
      button.setAttribute('aria-selected', String(selected))
      button.tabIndex = selected ? 0 : -1
    })
    trend.hidden = view === 'details'
    details.hidden = view !== 'details'
    enhanceServiceCenterDetailScrolling(main, pathname)
    const menu = main.querySelector('[data-aph-menu-outside-scroll="1"]')
    if (menu?.closest('.aph-service-center-scroll')) {
      details.parentElement?.insertBefore(menu, details)
    }

    if ((trend.textContent || '').includes('真实月度历史流水接口暂未接入')) {
      trend.classList.add('aph-history-unavailable')
      if (!trend.querySelector('.aph-history-status')) {
        const status = document.createElement('div')
        status.className = 'aph-history-status'
        status.innerHTML = '<div><strong>历史趋势暂不展示</strong><p>真实月度历史流水尚未接入，系统不会用演示曲线或复制快照代替。</p></div><span>当前可查看项目明细</span>'
        trend.append(status)
      }
    }
  }

  function enhanceServiceCenterDetailScrolling(main, pathname) {
    if (!main) return
    const serviceCenterDetailPaths = new Set(['/projects', '/payment', '/daily', '/collection'])
    if (!serviceCenterDetailPaths.has(pathname)) return

    const pagers = Array.from(main.querySelectorAll('.aph-project-pager, .aph-collection-pager'))
    if (pagers.length) {
      pagers.forEach(pager => pager.remove())
      main.querySelectorAll('tbody tr').forEach(row => { row.hidden = false })
    }
    ...
    main.querySelectorAll('table').forEach(table => {
      ...
      if (!host) return
      let scrollHost = host
      const hostContainsOnlyTable = host.children.length === 1 && host.firstElementChild === table
      if (!hostContainsOnlyTable && host.dataset.aphScrollScope !== 'table-only') {
        scrollHost = document.createElement('div')
        scrollHost.dataset.aphScrollScope = 'table-only'
        host.insertBefore(scrollHost, table)
        scrollHost.appendChild(table)
        host.classList.remove('aph-service-center-scroll')
        host.classList.add('aph-service-center-detail-shell')
      } else {
        scrollHost.dataset.aphScrollScope = 'table-only'
      }
      scrollHost.classList.add('aph-service-center-scroll')
      scrollHost.setAttribute('tabindex', '0')
      scrollHost.setAttribute('aria-label', '服务中心明细，可纵向和横向滚动')
    })
  }

  function reviewWorkbenchSection(element, root) {
    if (!element || !root) return null
    const zone = element.closest('.review-workbench-main, .review-workbench-aside, .review-workbench-side, aside')
    if (zone && root.contains(zone)) {
      let node = element
      while (node.parentElement && node.parentElement !== zone) node = node.parentElement
    ...
    while (node.parentElement && node.parentElement !== root) {
      ...
      const siblingPanels = Array.from(parent.children).filter(child => (
        child.querySelector?.('h1, h2, h3, h4')
      ))
      if (siblingPanels.length >= 2) return node
      node = parent
    }
    return element.closest('section, article')
  }

  const operationGroupDefinitions = {
    ai: {
      label: 'AI与机器人',
      headings: ['AI 审核健康', '审核机器人', '系统运行面板'],
    },
    execution: {
      label: '审核执行',
      headings: ['意见书导出看板', '执行漏斗', '执行驾驶舱', '提交人跟进视角', '执行催办看板'],
    },
    security: {
      label: '账号安全',
      headings: ['管理员上线入口', '账号安全态势'],
    },
    audit: {
      label: '日志审计',
      headings: ['审核效率趋势', '专业任务占比', '专业事项占比', '系统公告', '审核动作日志', '执行动作日志'],
    },
  }

  function setReviewWorkbenchUrlMode(mode, group) {
    const params = new URLSearchParams(window.location.search)
    params.set('view', 'workbench')
    params.set('mode', mode === 'operations' ? 'ops' : 'review')
    if (mode === 'operations') params.set('group', group)
    else params.delete('group')
    window.history.replaceState(null, '', `${window.location.pathname}?${params.toString()}${window.location.hash}`)
  }

  function enhanceReviewWorkbenchTaskFocus(main, root, mode) {
    const heroCopy = exactTextElements(main, '让每一次方案审核，都有标准、有依据、有结论')[0]
    const hero = reviewWorkbenchSection(heroCopy, root)
    if (hero) {
      hero.classList.add('aph-review-hero-compact')
      hero.dataset.reviewWorkbenchTodaySection = '1'
    }

    const statLabels = ['待复核方案', '已通过', '待修改', '机器人在线数']
    let stats = exactTextElements(main, statLabels[0])[0]?.closest('.grid')
    while (stats && stats !== root && !statLabels.every(label => (stats.textContent || '').includes(label))) {
      stats = stats.parentElement
    }
    if (stats && stats !== root) stats.dataset.reviewWorkbenchTodaySection = '1'

    const queueHeading = Array.from(main.querySelectorAll('h1,h2,h3,h4')).find(heading => (
      (heading.textContent || '').replace(/\s+/g, ' ').trim().startsWith('方案审核队列')
    ))
    const queue = reviewWorkbenchSection(queueHeading, root)
    if (queue) {
      queue.dataset.reviewTaskPriority = '1'
      queue.dataset.reviewWorkbenchTodaySection = '1'
      if (stats && stats.parentElement === queue.parentElement && stats.nextElementSibling !== queue) stats.after(queue)
    }

    const flowHeading = exactTextElements(main, '审核流程模型')[0]
    const flow = reviewWorkbenchSection(flowHeading, root)
    if (flow) flow.dataset.reviewWorkbenchTodaySection = '1'

    main.querySelectorAll('[data-review-workbench-today-section="1"]').forEach(section => {
      section.hidden = mode === 'operations'
    })
  }

  function updateReviewWorkbenchFreshness(main, switcher) {
    let freshness = switcher.querySelector('.aph-review-freshness')
    if (!freshness) {
      freshness = document.createElement('span')
      freshness.className = 'aph-review-freshness'
      freshness.setAttribute('role', 'status')
      freshness.setAttribute('aria-live', 'polite')
      switcher.insertBefore(freshness, switcher.querySelector('.aph-review-ops-groups'))
    }

    const aiHeading = exactTextElements(main, 'AI 审核健康')[0]
    const aiPanel = reviewWorkbenchSection(aiHeading, main)
    const match = (aiPanel?.textContent || '').match(/最近调用[：:]\s*([^·\n]+)/)
    if (!match) {
      freshness.className = 'aph-review-freshness is-unknown'
      freshness.textContent = 'AI记录更新时间：未提供'
      return
    }

    const raw = match[1].trim()
    const timestamp = Date.parse(raw.replaceAll('/', '-'))
    const stale = !Number.isFinite(timestamp) || Date.now() - timestamp > 24 * 60 * 60 * 1000
    freshness.className = `aph-review-freshness${stale ? ' is-stale' : ' is-fresh'}`
    freshness.textContent = `AI记录更新：${raw}${stale ? ' · 已过期' : ' · 正常'}`
  }

  function enhanceReviewWorkbenchDisclosure(main, pathname) {
    if (pathname !== '/review' || !main) return
    const params = new URLSearchParams(window.location.search)
    if ((params.get('view') || 'workbench') !== 'workbench') return
    const root = main.querySelector('.review-workbench-shell')
    if (!root) return

    let switcher = root.querySelector('[data-review-workbench-mode]')
    if (!switcher) {
      switcher = document.createElement('div')
      switcher.className = 'aph-page-view-switch aph-review-workbench-switch'
      switcher.dataset.reviewWorkbenchMode = 'switcher'
      switcher.setAttribute('role', 'tablist')
      switcher.setAttribute('aria-label', '研发审核首页视图')
      switcher.innerHTML = `<button type="button" role="tab" data-review-mode="today">今日审核</button><button type="button" role="tab" data-review-mode="operations">运行与安全</button><div class="aph-review-ops-groups" role="tablist" aria-label="运行与安全分组">${Object.entries(operationGroupDefinitions).map(([key, definition]) => `<button type="button" role="tab" data-review-ops-group="${key}">${definition.label}</button>`).join('')}</div>`
      root.insertBefore(switcher, root.firstChild)
      switcher.addEventListener('click', event => {
        const button = event.target.closest('[data-review-mode]')
        const groupButton = event.target.closest('[data-review-ops-group]')
        if (!button && !groupButton) return
        const nextMode = button?.dataset.reviewMode || 'operations'
        const nextGroup = groupButton?.dataset.reviewOpsGroup || window.sessionStorage.getItem('aph-review-ops-group') || 'ai'
        window.sessionStorage.setItem('aph-review-workbench-mode', nextMode)
        window.sessionStorage.setItem('aph-review-ops-group', nextGroup)
        setReviewWorkbenchUrlMode(nextMode, nextGroup)
        enhanceReviewWorkbenchDisclosure(main, pathname)
      })
    }

    Object.entries(operationGroupDefinitions).forEach(([group, definition]) => {
      definition.headings.forEach(label => exactTextElements(main, label).forEach(heading => {
        const section = reviewWorkbenchSection(heading, main)
        if (section && section !== switcher) {
          section.dataset.reviewWorkbenchSection = 'operations'
          section.dataset.reviewWorkbenchGroup = group
        }
      }))
    })

    const modeParam = params.get('mode')
    const mode = modeParam === 'ops'
      ? 'operations'
      : modeParam === 'review'
        ? 'today'
        : window.sessionStorage.getItem('aph-review-workbench-mode') === 'operations' ? 'operations' : 'today'
    const requestedGroup = params.get('group') || window.sessionStorage.getItem('aph-review-ops-group') || 'ai'
    const group = operationGroupDefinitions[requestedGroup] ? requestedGroup : 'ai'
    root.dataset.reviewWorkbenchMode = mode
    root.dataset.reviewWorkbenchGroup = group
    root.querySelectorAll('[data-review-mode]').forEach(button => {
      const selected = button.dataset.reviewMode === mode
      button.setAttribute('aria-selected', String(selected))
      button.tabIndex = selected ? 0 : -1
    })
    root.querySelectorAll('[data-review-ops-group]').forEach(button => {
      const selected = button.dataset.reviewOpsGroup === group
      button.setAttribute('aria-selected', String(selected))
      button.tabIndex = mode === 'operations' && selected ? 0 : -1
    })
    main.querySelectorAll('[data-review-workbench-section="operations"]').forEach(section => {
      section.hidden = mode !== 'operations' || section.dataset.reviewWorkbenchGroup !== group
    })
    enhanceReviewWorkbenchTaskFocus(main, root, mode)
    updateReviewWorkbenchFreshness(main, switcher)
  }

  function findAdminEditRow(button) {
    let current = button.parentElement
    while (current && current !== document.body) {
      const saves = Array.from(current.querySelectorAll('button')).filter(item => (item.textContent || '').trim() === '保存')
      if (saves.length === 1) return current
      current = current.parentElement
    }
    return button.parentElement
  }

  function enhanceAdminEditingSafety(pathname) {
    if (pathname !== '/admin') return
    const root = document.getElementById('root')
    if (!root) return
    const saves = Array.from(root.querySelectorAll('button')).filter(button => (button.textContent || '').trim() === '保存')
    if (!saves.length) return

    saves.forEach(button => {
      const row = findAdminEditRow(button)
      const dirty = row?.dataset.aphAdminDirty === '1'
      button.disabled = !dirty
      button.classList.toggle('aph-admin-save-locked', !dirty)
      button.classList.toggle('aph-admin-save-dirty', dirty)
      button.title = dirty ? '保存当前行修改' : '修改当前行后才会启用保存'
    })

    if (!root.querySelector('.aph-admin-safety-toolbar')) {
      const firstRow = findAdminEditRow(saves[0])
      let host = firstRow
      while (host?.parentElement && host.parentElement !== root) {
        const parentSaveCount = Array.from(host.parentElement.querySelectorAll('button')).filter(button => (button.textContent || '').trim() === '保存').length
        if (parentSaveCount > 1) break
        host = host.parentElement
      }
      const toolbar = document.createElement('div')
      toolbar.className = 'aph-admin-safety-toolbar'
      toolbar.innerHTML = '<strong>逐行编辑保护</strong><span>修改当前行后才会启用保存；其他行保持锁定，避免误触生产数据。</span>'
      host?.parentElement?.insertBefore(toolbar, host)
    }

    if (root.dataset.aphAdminSafetyBound !== '1') {
      const markDirty = event => {
        const candidate = event.target.closest('input,select,textarea,[contenteditable="true"]')
        if (!candidate) return
        let current = candidate.parentElement
        while (current && current !== root) {
          const rowSave = Array.from(current.querySelectorAll('button')).find(button => (button.textContent || '').trim() === '保存')
          if (rowSave) {
            current.dataset.aphAdminDirty = '1'
            rowSave.disabled = false
            rowSave.classList.remove('aph-admin-save-locked')
            rowSave.classList.add('aph-admin-save-dirty')
            rowSave.title = '保存当前行修改'
            break
          }
          current = current.parentElement
        }
      ...
      root.dataset.aphAdminSafetyBound = '1'
    }
  }

  function enhanceAiReportGateState(main, pathname) {
    if (pathname !== '/ai-report' || !main) return
    const blocked = window.__aphMonthlyReportBlocked || (main.textContent || '').includes('AI月报生成失败')
    if (!blocked) return

    const errorElements = exactTextElements(main, 'AI月报生成失败')
    const blockedCard = errorElements[0]?.closest('.card-soft')
    if (blockedCard && !blockedCard.querySelector('.aph-gate-empty')) {
      blockedCard.innerHTML = '<div class="aph-gate-empty"><strong>暂无可生成经营月报的有效项目事实</strong><p>当前项目经营事实层未通过正式数据发布门禁。系统不会用旧快照、演示项目或0值拼接月报；请先完成项目事实接入与发布验收。</p><p>收缴率和回款额页面的现有正式口径不受影响。</p></div>'
    }
    errorElements.forEach(errorElement => {
      if (errorElement.isConnected) errorElement.textContent = '暂无可生成经营月报的有效项目事实'
    })
    const textWalker = document.createTreeWalker(main, NodeFilter.SHOW_TEXT)
    let textNode = textWalker.nextNode()
    while (textNode) {
      if ((textNode.nodeValue || '').includes('AI月报生成失败')) {
    ...
    main.querySelectorAll('button').forEach(button => {
      if (!disabledLabels.has((button.textContent || '').trim())) return
      button.disabled = true
      button.title = '项目经营事实通过正式数据发布门禁后才可使用'
    })
  }

  function replaceLegacyBatchTerminology(root) {
    if (!root) return
    ...
    nodes.forEach(node => {
      ...
      const next = String(node.nodeValue || '')
        .replaceAll('P46受控批次', '正式数据发布门禁')
        .replaceAll('P46批次', '正式经营批次')
        .replaceAll('P46中转箱', '正式数据暂存区')
  ...
  function applyTheme() {
    ...
    const exactNavItems = [
      ...
      { href: '/arrears', label: '欠费经营分析', icon: 'chengbentongjifenxi.png' },
    ...
    exactSidebar?.querySelectorAll('a[data-aph-exact-href]').forEach(link => {
      ...
      const queryMatches = !targetQuery || Array.from(new URLSearchParams(targetQuery).entries()).every(
    ...
    enhanceServiceCenterDetailScrolling(main, path)
    enhanceCollectionDisclosure(main, path)
    enhanceReviewWorkbenchDisclosure(main, path)
    enhanceAdminEditingSafety(path)
    enhanceAiReportGateState(main, path)
    replaceLegacyBatchTerminology(main || document.getElementById('root'))
    ...
    if (header) header.classList.toggle('aph-admin-shell-header', path === '/admin')
    ...
    const currentHref = tabHrefForLocation(path, window.location.search)
      ...
      if (tabList && tabList.dataset.aphEventsBound !== '1') {
        tabList.addEventListener('click', event => {
          ...
          if (close) {
            ...
            const isActive = href === tabHrefForLocation(
