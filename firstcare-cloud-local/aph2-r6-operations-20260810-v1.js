(() => {
  ...
  const RELEASE = 'r6-operations-20260810-v1'
  ...
  function clarifyHomeGrouping() {
    ...
    document.querySelectorAll('span,div').forEach(node => {
      if (node.children.length === 0 && (node.textContent || '').trim() === '片区数 / 服务中心数') node.textContent = '经营分组 / 服务中心数'
    })
    document.querySelectorAll('article').forEach(card => {
      if (!/经营分组\s*\/\s*服务中心数/.test(card.textContent || '')) return
      let note = card.querySelector('small')
      if (!note) {
        note = document.createElement('small')
        card.appendChild(note)
      }
      note.textContent = '6个地理片区 + 3个特殊分组（公司 / 保洁 / 已撤场）'
    })
  }
