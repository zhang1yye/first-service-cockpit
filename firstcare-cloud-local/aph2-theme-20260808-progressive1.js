(() => {
  ...
  function hideCollectionAgingColumns(pathname) {
    ...
    main.querySelectorAll('table').forEach(table => {
      ...
      // 收缴率明细有 35 条。原组件的 max-h-[70vh] 会把表格裁成内部滚动区，
      // 在 APH 外壳中滚轮落到页面而不是表格，用户只能看到前几条和固定合计行。
      // 仅对本页的这张明细表取消最大高度，保留 table-scroll 的横向滚动能力。
      const scrollContainer = table.closest('.table-scroll')
      if (scrollContainer) {
        scrollContainer.style.setProperty('max-height', 'none', 'important')
        scrollContainer.dataset.aphCollectionRowsExpanded = 'true'
      }
    })
  }
    ...
    try {
      ...
      page.innerHTML = `
        ...
        <section class="aph-project-table-card">
          ...
          <div class="aph-project-pager" aria-label="项目档案分页">
            <span data-project-page-status></span>
            <button type="button" data-project-page-prev>上一页</button>
            <button type="button" data-project-page-next>下一页</button>
          </div>
      ...
      const pageStatus = page.querySelector('[data-project-page-status]')
      const previousPage = page.querySelector('[data-project-page-prev]')
      const nextPage = page.querySelector('[data-project-page-next]')
      ...
      const filter = () => {
        ...
        projectPageIndex = Math.min(projectPageIndex, pageCount - 1)
        const start = projectPageIndex * projectPageSize
        if (tbody) tbody.innerHTML = projectRowsHtml(visible.slice(start, start + projectPageSize))
        if (count) count.textContent = `${visible.length}个服务中心`
        if (pageStatus) pageStatus.textContent = visible.length ? `第 ${projectPageIndex + 1} / ${pageCount} 页 · 共 ${visible.length} 个` : '当前筛选无项目'
        if (previousPage) previousPage.disabled = projectPageIndex === 0
        if (nextPage) nextPage.disabled = projectPageIndex >= pageCount - 1
      }
      const resetAndFilter = () => { projectPageIndex = 0; filter() }
      area?.addEventListener('change', resetAndFilter)
      property?.addEventListener('change', resetAndFilter)
      query?.addEventListener('input', resetAndFilter)
      previousPage?.addEventListener('click', () => { if (projectPageIndex > 0) projectPageIndex -= 1; filter() })
      nextPage?.addEventListener('click', () => { projectPageIndex += 1; filter() })
      filter()
  ...
  function enhanceCollectionDisclosure(main, pathname) {
    ...
    enhanceCollectionPagination(details)
  ...
  function enhanceCollectionPagination(details) {
    if (!details) return
    const table = details.querySelector('table')
    const body = table?.tBodies?.[0]
    if (!table || !body) return

    const collectionPageSize = 15
    const rows = Array.from(body.rows)
    const totalRows = rows.filter(row => (row.textContent || '').includes('华北合计'))
    const dataRows = rows.filter(row => !totalRows.includes(row))
    if (!dataRows.length) return

    const signature = dataRows.map(row => (row.cells[0]?.textContent || '').trim()).join('|')
    if (details.dataset.aphCollectionPageSignature !== signature) {
      details.dataset.aphCollectionPageSignature = signature
      details.dataset.aphCollectionPage = '0'
    }

    const totalPages = Math.max(1, Math.ceil(dataRows.length / collectionPageSize))
    const requestedPage = Number.parseInt(details.dataset.aphCollectionPage || '0', 10)
    const page = Math.min(Math.max(Number.isFinite(requestedPage) ? requestedPage : 0, 0), totalPages - 1)
    details.dataset.aphCollectionPage = String(page)
    const start = page * collectionPageSize
    const end = start + collectionPageSize

    dataRows.forEach((row, index) => {
      row.hidden = index < start || index >= end
    })
    totalRows.forEach(row => { row.hidden = false })

    let pager = details.querySelector('.aph-collection-pager')
    if (!pager) {
      pager = document.createElement('div')
      pager.className = 'aph-project-pager aph-collection-pager'
      pager.innerHTML = '<span data-collection-page-status></span><button type="button" data-collection-page-prev>上一页</button><button type="button" data-collection-page-next>下一页</button>'
      const tableHost = table.closest('.overflow-x-auto') || table
      tableHost.insertAdjacentElement('afterend', pager)
      pager.addEventListener('click', event => {
        const previous = event.target.closest('[data-collection-page-prev]')
        const next = event.target.closest('[data-collection-page-next]')
        if (!previous && !next) return
        const current = Number.parseInt(details.dataset.aphCollectionPage || '0', 10) || 0
        details.dataset.aphCollectionPage = String(current + (next ? 1 : -1))
        enhanceCollectionPagination(details)
      })
    }

    const status = pager.querySelector('[data-collection-page-status]')
    const statusText = `当前第 ${page + 1}/${totalPages} 页 · 共 ${dataRows.length} 条项目明细`
    if (status && status.textContent !== statusText) status.textContent = statusText
    const previous = pager.querySelector('[data-collection-page-prev]')
    const next = pager.querySelector('[data-collection-page-next]')
  ...
  function enhanceReviewWorkbenchDisclosure(main, pathname) {
    ...
    if (!switcher) {
      ...
      switcher.innerHTML = '<button type="button" role="tab" data-review-mode="today">今日审核</button><button type="button" role="tab" data-review-mode="operations">运行与安全</button>'
      root.insertBefore(switcher, root.firstChild)
      switcher.addEventListener('click', event => {
        const button = event.target.closest('[data-review-mode]')
        if (!button) return
        window.sessionStorage.setItem('aph-review-workbench-mode', button.dataset.reviewMode)
    ...
    operationHeadings.forEach(label => {
      exactTextElements(root, label).forEach(heading => {
        const section = reviewWorkbenchSection(heading, root)
        if (section && section !== switcher) section.dataset.reviewWorkbenchSection = 'operations'
      })
    })

    const mode = window.sessionStorage.getItem('aph-review-workbench-mode') === 'operations' ? 'operations' : 'today'
    ...
    root.querySelectorAll('[data-review-workbench-section="operations"]').forEach(section => {
  ...
  function enhanceAiReportGateState(main, pathname) {
    ...
    const blockedCard = exactTextElements(main, 'AI月报生成失败')[0]?.closest('.card-soft')
