...
CHUNK = ROOT / "firstcare-cloud-local/assets/cockpit-r77-region-wide-scope-20260813-v1/chunk-R65SIMPLEADMIN.js"
CSS = ROOT / "firstcare-cloud-local/aph2-r76-member-scope-20260813-v1.css"
R77_CSS = ROOT / "firstcare-cloud-local/aph2-r77-region-wide-scope-20260813-v1.css"
OUT = ROOT / "docs/qa/frontend-skill-cloud-20260813/r77-region-wide-scope-shadow"
TARGET = os.environ.get("R77_QA_TARGET", "shadow")
if TARGET == "production":
    OUT = ROOT / "docs/qa/frontend-skill-cloud-20260813/r77-region-wide-scope-production"
...
def run_viewport(browser, auth_token, width, height):
    ...
    def route_handler(route):
        ...
        if TARGET == "shadow" and request.url.endswith("/assets/cockpit-r76-member-scope-20260813-v1/chunk-R65SIMPLEADMIN.js"):
    ...
    if TARGET == "shadow":
        ...
        page.add_style_tag(path=str(R77_CSS))
    ...
    role_select.select_option("region_manager")
    assert page.locator(".r65-member-form .r76-all-scope", has_text="所有片区").count() == 1
    assert page.locator(".r65-member-form .r76-all-scope", has_text="所有服务中心").count() == 1
    assert page.locator(".r65-member-form .r76-center-select").count() == 0
    page.screenshot(path=str(OUT / f"{width}x{height}-region-wide.png"), full_page=True)
    ...
    page.locator(".r65-member-form input").nth(0).fill("r77-shadow-member")
    page.locator(".r65-member-form input[type=password]").fill("R77-Shadow-2026")
    role_select.select_option("region_manager")
    ...
    assert post["payload"]["role"] == "region_manager"
    assert post["payload"]["area_scope"] == ""
    assert post["payload"]["service_center_scope"] == []
    ...
    result = {
        ...
        "roles": role_options,
        "regionWideLabels": ["所有片区", "所有服务中心"],
        "restrictedArea": areas[0],
        "restrictedSelectedCenters": selected_centers,
        "capturedWrite": post,
