#!/usr/bin/env python3
"""R65 系统管理浏览器验收：真实候选资产 + 可审计的 API 合同模拟。"""
...
BASE_URL = os.environ.get("R65_QA_BASE_URL", "http://127.0.0.1:43165/")
...
class ApiHarness:
    def __init__(self, *, center_failure: bool = False, auth_role: str = "admin") -> None:
        self.center_failure = center_failure
        self.auth_role = auth_role
        self.requests: list[dict] = []
        self.payment_conflict_once = True
    ...
    def handle(self, route: Route) -> None:
        ...
        if path == "/api/projects/1":
            project = {
                ...
                "annual_income": 100, "annual_cost": 50, "ytd_income": 70, "ytd_cost": 30,
                "receivable": 100, "received": 88, "quality_score": 90,
                "safety_incidents": 0, "customer_satisfaction": 90, "complaint_count": 0,
                "area_sqm": 1000, "units": 100,
            }
            return self._json(route, {"project": project, "collectionRate": 88, "profitRate": 57, "areaAvg": {}})
        if path == "/api/data-quality/project-gate":
            return self._json(route, {"ready": True, "status": "ready", "scoped": True, "projectCount": 1})
        if path == "/api/alerts":
        ...
        if path == "/api/payments/1" and method == "PUT":
            if self.payment_conflict_once:
                self.payment_conflict_once = False
                return self._json(route, {"error": "数据版本冲突"}, 409)
        ...
        if path == "/api/trends":
            return self._json(route, [{"m": "2026-08", "华北汇总": 88}])
        ...
        if path == "/api/governance/report-archives":
            return self._json(route, {"rows": [{
        ...
        if path == "/api/governance/report-archives/1":
            return self._json(route, {"id": 1, "report_date": "2026-08", "status": "已归档"})
...
def install_login(page, *, role: str = "admin", pathname: str = "/admin") -> None:
    user = {
        "id": 1,
...
def assert_desktop_admin(browser) -> Path:
    ...
    payment_region = page.get_by_role("region", name="回款额维护")
    assert payment_region.get_attribute("tabindex") == "0"
    payment_region.focus()
    assert page.evaluate("document.activeElement?.getAttribute('aria-label')") == "回款额维护"
    ...
    budget = page.get_by_role("spinbutton", name=f"{CENTER_A}年度预算")
    budget.fill("120")
    page.get_by_role("button", name="保存修改", exact=True).click()
    page.get_by_role("alert").wait_for()
    assert "数据版本冲突；已重新加载服务器最新值" in page.get_by_role("alert").inner_text()
    payment_put = next(item for item in harness.requests if item["path"] == "/api/payments/1")
    assert payment_put["body"] == {"annualBudget": 120, "version": 2}
    ...
    deleted = next(item for item in harness.requests if item["method"] == "DELETE")
    ...
    screenshot = Path(os.environ.get("R65_QA_SCREENSHOT", "/tmp/r65-admin-desktop.png"))
...
def assert_mobile_and_route_guard(browser) -> Path:
    ...
    screenshot = Path(os.environ.get("R65_QA_MOBILE_SCREENSHOT", "/tmp/r65-admin-mobile.png"))
    admin_nav.screenshot(path=str(screenshot))
    mobile_table = page.get_by_role("region", name="回款额维护")
...
def assert_member_mobile_routes_and_badge(browser) -> None:
    ...
    for pathname in allowed:
        print(f"R65 member route: {pathname}", flush=True)
        if pathname != "/":
...
def main() -> None:
    with sync_playwright() as playwright:
        ...
        assert_member_mobile_routes_and_badge(browser)
        browser.close()
    print(json.dumps({
        "ok": True,
        "checks": [
