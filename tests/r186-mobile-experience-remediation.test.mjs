...
const release = 'cockpit-r186-mobile-experience-remediation-20260901-v1'
...
const bootstrap = fs.readFileSync(path.join(payload, 'releases', release, 'aph2-r186-mobile-experience-remediation-20260901-v1.js'), 'utf8')
...
test('R186继续保留片区执行排名到回款额执行评估的联动', () => {
  ...
  assert.match(index, new RegExp(`type="module"[^>]+releases/${release}/aph2-r186-mobile-experience-remediation-20260901-v1\\.js`))
...
test('R186片区筛选后按正式中心收缴率加权显示片区综合收缴率', () => {
...
test('R186使用稳定属性控制收缴视图，不再依赖动态片区标题', () => {
...
test('R186回款移动卡显示核心执行指标且桌面表格继续保留', () => {
