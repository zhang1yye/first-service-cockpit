...
const release = 'cockpit-r64-ai-header-all-viewports-v2-20260813-023341'
...
const baselinePath = path.join(
  root,
  'release-candidates/cockpit-r63-no-header-explainers-v2-20260813-022438/payload/web/index.html',
)
...
test('R64 候选只含两项产品资产、manifest 与默认只读发布器', () => {
  ...
  assert.deepEqual(manifest.productionBaseline['index.html'], {
    size: 7326,
    sha256: '63d67bcb8b750391f4f439df4272fab1b8879e0b3ee809a775a9fc41c34aafe2',
  })
  assert.deepEqual(manifest.expectedResult, {
    size: 7426,
    sha256: 'bced9e2a39363c8dd15412bcdb4b5e437f7f4a8abeceb8c124762fc9f4b770e3',
  })
...
test('R64 v2 只删除 R59 一行，保留 R63 v2/R65 引用并在精确锚点后增加 CSS/JS', () => {
...
`
  ...
  assert.equal(result.beforeSize, 7326)
  ...
  const cssAnchor = manifest.entryTransform.insertCssAfter
  ...
  assert.ok(result.after.indexOf(cssAnchor) < result.after.indexOf(r64Css))
...
test('R64 发布器具备基线阻断、全局锁、路径防护、资产先行、只读 DB 与失败回滚', () => {
  ...
  for (const required of [
    '63d67bcb8b750391f4f439df4272fab1b8879e0b3ee809a775a9fc41c34aafe2',
