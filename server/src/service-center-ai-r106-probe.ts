...
export const SERVICE_CENTER_AI_PROMPT_VERSION = 'service-center-hermes-v4-center-specific-advice'
...
const MODEL_DANGEROUS_CONTENT = /(?:银行卡|银行账户|卡号|密码|口令|验证码|身份证|证件|复印件|口头说明|隐私|公开|公示|曝光|泄露|停水|停电|断水|断电|威胁|恐吓|骚扰|施压|围堵|堵门)/
const CONTROLLED_COORDINATION_ROLES = [
  '服务中心收费岗位', '收费经办岗位', '项目负责人', '运营负责人', '片区负责人', '区域负责人',
  '部门负责人', '项目经理', '收费岗位', '客服管家', '物业管家', '财务岗位', '财务人员', '法务岗位',
  '法务人员', '客服岗位', '收费岗', '收费员', '客服', '财务', '法务',
] as const
...
const APPROVED_ESCALATION_ACTIONS = ['转交', '上报', '升级'] as const
const APPROVED_ESCALATION_PROCESSES = [
  '争议处理流程', '催缴升级流程', '法务处理流程', '法务流程', '审批流程', '升级流程',
  '分类处理',
] as const
...
function boundedConcurrency(options: ServiceCenterAiOptions): number {
  const raw = options.concurrency ?? Number(process.env.SERVICE_CENTER_AI_CONCURRENCY || 2)
  return Math.max(1, Math.min(4, Number.isFinite(raw) ? Math.trunc(raw) : 2))
}
...
function hasExactAttentionJudgement(row: ServiceCenterAnalysisRow, modelText: string): boolean {
  ...
  if (matched[2] && !hasOnlyDeclaredSignals(row, matched[2])) return false
  if (matched[1] && matched[2]) return false
  const focusLine = matched[3].trim()
  const expected = new RegExp(`^围绕[“\"]?${escapeRegExp(focusLabel)}[”\"]?推进可转化欠费款项(?:尽快)?到账\\s*[。.]?$`)
  return expected.test(focusLine)
}
...
function parseExecutionCard(modelText: string): ExecutionCardFields | null {
  ...
  const body = text.slice((marker.index || 0) + marker[0].length)
  // AI建议正文只允许可选固定标题和恰好五个字段。任何夹带责任人、
  // 已完成声明或能力评价的额外行都必须整段拒绝，避免被原样展示。
  const matched = body.match(/^\s*(?:一线执行卡[ \t]*\r?\n\s*)?先做什么[ \t]*[:：][ \t]*(?:\r?\n)?([\s\S]*?)\r?\n\s*谁来协同[ \t]*[:：][ \t]*([^\r\n]+)\r?\n\s*所需材料[ \t]*[:：][ \t]*([^\r\n]+)\r?\n\s*完成凭证[ \t]*[:：][ \t]*([^\r\n]+)\r?\n\s*升级条件[ \t]*[:：][ \t]*([^\r\n]+)\s*$/)
  if (!matched) return null
  return {
    action: matched[1].trim(),
    coordination: matched[2].trim(),
    materials: matched[3].trim(),
    completionEvidence: matched[4].trim(),
    escalation: matched[5].trim(),
  }
}
...
function hasThreeCenterSpecificActions(value: string): boolean {
  ...
  return lines.every((line, index) => {
    ...
    const stageMatches = FRONTLINE_ACTION_STAGES.map(pattern => pattern.test(action))
    const allowed = hasOnlyCollectionActions(action)
    return allowed
      && stageMatches[index]
...
function hasOnlyApprovedEscalation(value: string): boolean {
  ...
  // 顺序必须是：条件 -> 则/就 -> 升级动作 -> 批准岗位 -> 批准流程。
  // 先写升级或流程、再把“若拒付则”拼到句尾的文本不得通过。
  const triggerMatches = removeApprovedPhrases(clean, APPROVED_ESCALATION_TRIGGERS).matched
  if (triggerMatches !== 1) return false
  return new RegExp(
    `^(?:若|如)\\s*(?:出现|发生|存在)?\\s*${triggerList}\\s*(?:，|,)?\\s*(?:则|就)\\s*(?:${actionPattern})\\s*(?:${rolePattern})\\s*(?:按|进入|启动|通过|依照)?\\s*(?:${processPattern})\\s*[。.]?$`,
  ).test(clean)
}
...
export function buildServiceCenterHermesInput(
  ...
  const question = hasGroundedAdvice
    ? `请针对${row.center}的已核验未达标项生成一份有针对性的AI建议，不要复制作业标准或罗列全部可选动作。严格输出“AI判断：”，可先用“未达标项：”逐字列出系统已有信号，再输出“回款重点：围绕<关注标签>推进可转化欠费款项到账。”，其中<关注标签>必须原样替换为focusLabel。随后输出“AI建议：”和“一线执行卡”。“先做什么：”后换行且只写“动作一：”“动作二：”“动作三：”三步，分别对应筛选/分类、联系/沟通、跟进/催缴三个阶段；${actionDirection}每一步只选择与本中心首要信号最相关的一个动作，不写通用口号。谁来协同只选1至2个明确岗位；所需材料只选2至3项必要材料；完成凭证只选1项；升级条件只选拒付、争议或承诺未兑现中的1种，并写明升级动作、批准岗位和批准流程。只能使用系统事实与已批准知识，不得补造数字、原因、完成状态、人名或期限；整张建议末尾标注最相关[K编号]。`
...
export async function analyzeServiceCenterRow(
  ...
  const refine = options.refine || refineWithHermes
  const sleep = options.sleep || ((milliseconds: number) => new Promise(resolve => setTimeout(resolve, milliseconds)))
  const deadline = now() + Math.max(1_000, hermes.timeoutMs || DEFAULT_TIMEOUT_MS)
  let response = await oneRefinement(input, hermes, refine, Math.max(1_000, deadline - now()))
  const retryableHttp = response.observation.kind === 'http'
    && [429, 502, 503, 504].includes(Number(response.observation.status))
  if (!response.result && retryableHttp && deadline - now() > 1_500) {
    await sleep(500)
    response = await oneRefinement(input, hermes, refine, Math.max(1_000, deadline - now()))
  }
  if (!response.result) {
    const rejected = response.observation.kind === 'ok'
    const errorCode = response.observation.kind === 'http'
  ...
  const safeOpinion = row.signals.length ? aiOpinion : { ...aiOpinion, recommendations: null }
  return {
    inputSha256: options.inputSha256,
    normalizedCenter: row.normalizedCenter,
    businessDate: options.businessDate,
